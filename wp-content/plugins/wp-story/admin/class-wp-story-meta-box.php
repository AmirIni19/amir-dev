...
class Wp_Story_Meta_Box {
	...
	public function render_side_rate() {
		...
		echo '<a class="rate-link" href="https://tr.wordpress.org/plugins/wp-story/#reviews" target="_blank">' . esc_html__( 'لطفا به افزونه ما امتیاز دهید!', 'wp-story' ) . '</a>';
	}
	...
	public function render_meta_box_content( $post ) {
		...
		<div class="ws-wrapper">
			<div class="ws-repeater-wrapper">
				...
				foreach ( $stories as $story ) { ?>
					<div class="ws-row" id="ws-repeater">
						<div class="ws-col-3">
							<div class="ws-inner">
								<label><?php esc_html_e( 'متن لینک استوری', 'wp-story' ); ?></label>
								<input type="text" name="wp_story_items_text[]" value="<?php echo esc_attr( $story['text'] ); ?>">
								<button type="button" class="story-remove">
									<span class="dashicons dashicons-trash"></span><?php esc_html_e( 'حذف', 'wp-story' ); ?>
								</button>
							</div>
						</div>
						<div class="ws-col-3">
							<div class="ws-inner">
								<label><?php esc_html_e( 'لینک استوری', 'wp-story' ); ?></label>
								<input type="text" name="wp_story_items_link[]" value="<?php echo esc_attr( $story['link'] ); ?>">
								<label class="wps-cb-label"><?php esc_html_e( 'تب جدید', 'wp-story' ); ?>
						...
						<div class="ws-col-3">
							<div class="ws-inner">
								<label><?php esc_html_e( 'تصویر یا ویدیو استوری', 'wp-story' ); ?></label>
								...
								<?php if ( isset( $story['src'] ) ): ?>
									<img src="<?php echo esc_url( $story['src'] ); ?>" alt="<?php esc_attr_e( 'تصویر استوری', 'wp-story' ); ?>">
								<?php endif; ?>
							</div>
						</div>
						<div class="ws-col-3">
							<div class="ws-inner">
								<label for=""><?php esc_html_e( 'مدت زمان (ثانیه)', 'wp-story' ); ?></label>
								<div class="pro-wrapper">
									<input type="number" name="">
									<a href="https://codecanyon.net/item/wp-story-premium/27546341" target="_blank" class="pro-info"><?php esc_html_e( 'پرو', 'wp-story' ); ?></a>
								</div>
							</div>
						</div>
					</div>
			...
			<button type="button" class="story-add">
				<span class="dashicons dashicons-plus"></span><?php esc_html_e( 'افزودن استوری', 'wp-story' ); ?>
			</button>
		</div>
			...
			(function ($) {
				...
				$(document).on('click', '.ws-uploader', function (e) {
					...
					let custom_uploader = wp.media.frames.file_frame = wp.media({
						title: '<?php esc_html_e( 'انتخاب رسانه', 'wp-story' ); ?>',
						button: {
							text: '<?php esc_html_e( 'انتخاب رسانه', 'wp-story' ); ?>'
						},
						multiple: false,
						library: {
							type: ['image', 'video']
