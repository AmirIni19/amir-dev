...
if ( ! function_exists( 'flatsome_posted_on' ) ) :
	...
	function flatsome_posted_on() {
		...
		echo sprintf(
			...
			wp_kses_post( _x( '<span class="posted-on">انتشار در تاریخ %1$s</span> <span class="byline">توسط %2$s</span>', 'post date by post author', 'flatsome' ) ),
        ...
        if( $total > 1 )  {
            ...
            if( is_array( $pages ) ) {
                ...
                foreach ( $pages as $page ) {
					...
					$page = str_replace( '<a class="next page-number', '<a aria-label="' . esc_attr__( 'بعدی', 'flatsome' ) . '" class="next page-number', $page );
					$page = str_replace( '<a class="prev page-number', '<a aria-label="' . esc_attr__( 'قبلی', 'flatsome' ) . '" class="prev page-number', $page );
					echo "<li>$page</li>";
                }
               echo '</ul>';
            }
